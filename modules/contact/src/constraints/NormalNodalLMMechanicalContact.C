    ...
    _displaced_problem(_fe_problem.getDisplacedProblem().get())
{
  ...
  if (disp_type.order.get_order() != lm_type.order.get_order())
    mooseWarning("The mechanical contact Lagrange multiplier has to be the same order as the mesh. "
                 "NormalNodalLMMechanicalContact is applied pointwise at every slave node. This is "
                 "unssupported and may lead to termination due to bad accesses");
}
