...
#include "GasMixPCNSFVLaxFriedrichsBC.h"
#include "NS.h"
#include "SinglePhaseFluidProperties.h"
#include "Function.h"

registerMooseObject("MooseApp", GasMixPCNSFVLaxFriedrichsBC);

InputParameters
GasMixPCNSFVLaxFriedrichsBC::validParams()
{
  ...
  params.addRequiredCoupledVar("fractions", "Secondary mass fractions for gas species");
...
GasMixPCNSFVLaxFriedrichsBC::GasMixPCNSFVLaxFriedrichsBC(const InputParameters & params)
  : FVFluxBC(params),
    _fluid(getUserObject<SinglePhaseFluidProperties>(NS::fluid)),
    _fractions(adCoupledValue("fractions")),
    _var_grad_fractions(adCoupledGradient("fractions")), 
...
void
GasMixPCNSFVLaxFriedrichsBC::computeResidual(const FaceInfo & fi)
{
  _face_info = &fi;
  _normal = fi.normal();
...
void
GasMixPCNSFVLaxFriedrichsBC::computeJacobian(const FaceInfo & fi)
{
  _face_info = &fi;
  _normal = fi.normal();
  auto ft = fi.faceType(_var.name());
...
ADReal
GasMixPCNSFVLaxFriedrichsBC::computeQpResidual()
{
  ...
  std::vector<ADReal> mf_interior(1);
  std::vector<ADReal> mf_boundary(1);
  mf_interior[0] = 0.7;
  mf_boundary[0] = 0.7;  
  // -------------- Modified
  std::cout<<pressure_interior<<std::endl;
  std::cout<<rho_interior<<std::endl;
  std::cout<<mf_interior[0]<<std::endl;
  std::cout<<T_fluid_interior<<std::endl;
  const auto e_interior = _fluid.e_from_p_rho_X(pressure_interior, rho_interior, mf_interior);
  // -----------------------
  ...
  const auto rho_boundary =
      _implicit_state_var ? rho_interior : _fluid.rho_from_p_T_X(pressure_boundary, T_fluid_boundary, mf_boundary);
  // --------------------------------
  ...
  // -------------- Modified
  const auto e_boundary = _fluid.e_from_p_rho_X(pressure_boundary, rho_boundary, mf_boundary);
    // --------------
  ...
  ADReal dc_interior_dv, dc_interior_de, dc_interior_dx, dc_boundary_dv, dc_boundary_de, dc_boundary_dx;
  
  _fluid.c_from_v_e_X(specific_volume_interior, e_interior, mf_interior, c_interior, dc_interior_dv, dc_interior_de, dc_interior_dx);
  _fluid.c_from_v_e_X(specific_volume_boundary, e_boundary, mf_boundary, c_boundary, dc_boundary_dv, dc_boundary_de, dc_boundary_dx);
  c_interior.derivatives() = dc_interior_dv.value() * specific_volume_interior.derivatives() + dc_interior_de.value() * e_interior.derivatives() + dc_interior_dx.value() * _scalar_elem[_qp].derivatives();
  c_boundary.derivatives() = dc_boundary_dv.value() * specific_volume_boundary.derivatives() + dc_boundary_de.value() * e_boundary.derivatives() + dc_boundary_dx.value() * _scalar_neighbor[_qp].derivatives();
// --------------------------------------------------------------------------------
