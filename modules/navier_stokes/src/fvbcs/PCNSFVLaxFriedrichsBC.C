...
PCNSFVLaxFriedrichsBC::computeResidual(const FaceInfo & fi)
{
  ...
  if (ft == FaceInfo::VarFaceNeighbors::ELEM)
    ...
    mooseError("A PCNSFVLaxFriedrichsBC is being triggered on an internal face with "
               "centroid: ",
               fi.faceCentroid());
  else
    mooseError("A PCNSFVLaxFriedrichsBC is being triggered on a face which does not "
               "connect to a block ",
...
PCNSFVLaxFriedrichsBC::computeQpResidual()
{
  ...
  auto sup_vel_y_interior = out_of_elem ? _sup_vel_y_elem[_qp] : _sup_vel_y_neighbor[_qp];
  auto sup_vel_z_interior = out_of_elem ? _sup_vel_z_elem[_qp] : _sup_vel_z_neighbor[_qp];
  ...
  const auto sup_vel_x_boundary =
      _svel_provided ? ADReal(_svel_function->vectorValue(_t, _face_info->faceCentroid())(0)) /
  ...
  const auto sup_vel_y_boundary =
      _svel_provided ? ADReal(_svel_function->vectorValue(_t, _face_info->faceCentroid())(1)) /
                           (_velocity_function_includes_rho ? rho_boundary : ADReal(1.))
                     : out_of_elem ? _sup_vel_y_neighbor[_qp] : _sup_vel_y_elem[_qp];
  const auto sup_vel_z_boundary =
      _svel_provided ? ADReal(_svel_function->vectorValue(_t, _face_info->faceCentroid())(2)) /
                           (_velocity_function_includes_rho ? rho_boundary : ADReal(1.))
