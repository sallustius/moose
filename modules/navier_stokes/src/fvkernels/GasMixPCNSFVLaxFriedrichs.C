...
#include "GasMixPCNSFVLaxFriedrichs.h"
#include "NS.h"
#include "SinglePhaseFluidProperties.h"

registerMooseObject("MooseApp", GasMixPCNSFVLaxFriedrichs);

InputParameters
GasMixPCNSFVLaxFriedrichs::validParams()
{
...
GasMixPCNSFVLaxFriedrichs::GasMixPCNSFVLaxFriedrichs(const InputParameters & params)
  : FVFluxKernel(params),
...
void
GasMixPCNSFVLaxFriedrichs::computeResidual(const FaceInfo & fi)
{
...
void
GasMixPCNSFVLaxFriedrichs::computeJacobian(const FaceInfo & fi)
{
  if (skipForBoundary(fi))
    return;
...
void
GasMixPCNSFVLaxFriedrichs::computeAValues()
{
  ...
  // -------------- Modified
  std::vector<ADReal> mf_elem(1);
  std::vector<ADReal> mf_neighbor(1);
  mf_elem[0] = _scalar_elem[_qp];
  mf_neighbor[0] = _scalar_neighbor[_qp];

  ADReal c_elem=0.0, c_neighbor=0.0;
  ADReal dc_elem_dv, dc_elem_de, dc_elem_dx, dc_neighbor_dv, dc_neighbor_de, dc_neighbor_dx;
  _fluid.c_from_v_e_X(specific_volume_elem.value(), _e_elem[_qp].value(), mf_elem, c_elem.value(), dc_elem_dv, dc_elem_de, dc_elem_dx);
  _fluid.c_from_v_e_X(specific_volume_neighbor.value(), _e_neighbor[_qp].value(), mf_neighbor, c_neighbor.value(), dc_neighbor_dv, dc_neighbor_de, dc_neighbor_dx);
  c_elem.derivatives() = dc_elem_dv.value() * specific_volume_elem.derivatives() + dc_elem_de.value() * _e_elem[_qp].derivatives() + dc_elem_dx.value() * _scalar_elem[_qp].derivatives();
  c_neighbor.derivatives() = dc_neighbor_dv.value() * specific_volume_neighbor.derivatives() + dc_neighbor_de.value() * _e_neighbor[_qp].derivatives() + dc_neighbor_dx.value() * _scalar_neighbor[_qp].derivatives();
...
ADReal
GasMixPCNSFVLaxFriedrichs::computeQpResidual()
{
  computeAValues();
