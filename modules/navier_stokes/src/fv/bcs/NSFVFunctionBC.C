...
ADReal
NSFVFunctionBC::coeffCalculator(const Elem & elem)
{
  ADReal coeff = 0;
...
{
  FVFluxBC::interpolate(InterpMethod::Average, v, elem_v, neighbor_v);
  ...
  if (m == InterpMethod::RhieChow)
  {
    ...
    if (_face_info->neighborPtr())
    {
      ...
      FVFluxBC::interpolate(InterpMethod::Average, face_a, elem_a, neighbor_a);
      FVFluxBC::interpolate(InterpMethod::Average,
                            face_volume,
