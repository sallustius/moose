...
PorousPrimitiveVarMaterial::PorousPrimitiveVarMaterial(const InputParameters & params)
  : Material(params),
    ...
    _grad_sup_vel_y(_grad_var_sup_vel_y
                        ? &declareADProperty<RealVectorValue>(NS::grad(NS::superficial_velocity_y))
                        : nullptr),
    _sup_vel_z(declareADProperty<Real>(NS::superficial_velocity_z)),
    _grad_sup_vel_z(_grad_var_sup_vel_z
                        ? &declareADProperty<RealVectorValue>(NS::grad(NS::superficial_velocity_z))
                        : nullptr),
    // properties: for viz
...
PorousPrimitiveVarMaterial::computeQpProperties()
{
  ...
  if (_grad_var_sup_vel_y)
    (*_grad_sup_vel_y)[_qp] = (*_grad_var_sup_vel_y)[_qp];
  if (_grad_var_sup_vel_z)
