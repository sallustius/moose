...
class GasMixPorousMixedVarMaterial : public Material
{
public:
  GasMixPorousMixedVarMaterial(const InputParameters & parameters);
...
protected:
  ...
  const ADVariableGradient & _grad_var_fraction;
  const ADVariableValue & _fraction_dot;
  ...
  ADMaterialProperty<Real> & _sup_rho_f_dot;
};
