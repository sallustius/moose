...
ADCoupledVelocityMaterial::validParams()
{
  ...
  params.addParam<bool>("fv", false, "Whether the coupled variables are FV variables");
...
ADCoupledVelocityMaterial::ADCoupledVelocityMaterial(const InputParameters & parameters)
  : Material(parameters),
    ...
    _vel_x(getParam<bool>("fv") ? adCoupledFVValue("vel_x") : adCoupledValue("vel_x")),
    _vel_y(isParamValid("vel_y")
               ? (getParam<bool>("fv") ? &adCoupledFVValue("vel_y") : &adCoupledValue("vel_y"))
               : nullptr),
    _vel_z(isParamValid("vel_z")
               ? (getParam<bool>("fv") ? &adCoupledFVValue("vel_z") : &adCoupledValue("vel_z"))
               : nullptr),
    _rho(getParam<bool>("fv") ? adCoupledFVValue("rho") : adCoupledValue("rho"))
{
}
...
ADCoupledVelocityMaterial::computeQpProperties()
{
  ...
  if (_vel_y)
  {
    ...
    _rho_v[_qp] = _rho[_qp] * velocity(1);
  }

  if (_vel_z)
  {
    velocity(2) = (*_vel_z)[_qp];
    _rho_w[_qp] = _rho[_qp] * velocity(2);
  }
}
