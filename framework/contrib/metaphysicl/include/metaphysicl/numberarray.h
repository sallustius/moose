...
{
  typedef NumberArray<N, typename OuterProductType<S, T, reverseorder>::supertype> supertype;
};
...
public:
  ...
  NumberArray(const T & val) { std::fill(_data, _data + N, val); }

  NumberArray(const T * vals) { std::copy(vals, vals + N, _data); }

  template <typename T2>
  NumberArray(NumberArray<N, T2> src)
  {
    if (N)
      std::copy(&src[0], &src[0] + N, _data);
  }
  ...
  T & raw_at(std::size_t i) { return _data[i]; }

  const T & raw_at(std::size_t i) const { return _data[i]; }
  ...
  template <typename T2>
  NumberArray<N, typename DotType<T, T2>::supertype> dot(const NumberArray<N, T2> & a) const
  {
  ...
  {
    typename OuterProductType<NumberArray<N, T>, NumberArray<N, T2>>::supertype returnval;
...
inline typename CompareTypes<T, T2>::supertype
      ...
      typename CompareTypes<
          ...
          typename CompareTypes<
              ...
              typename CompareTypes<
                  ...
                  typename CompareTypes<
                      Data5,
                      typename CompareTypes<Data6, typename CompareTypes<Data7, Data8>::supertype>::
...
{
  NumberArray<N, typename SumType<T>::supertype> returnval = 0;
...
#define NumberArray_std_unary(funcname)                                                            \
  template <std::size_t N, typename T>                                                             \
  inline NumberArray<N, T> funcname(NumberArray<N, T> a)                                           \
  {                                                                                                \
    for (std::size_t i = 0; i != N; ++i)                                                           \
...
#define NumberArray_std_binary_abab(funcname, atype, btype, abtypes, aarg, barg)                   \
  template <std::size_t N, typename T, typename T2>                                                \
  inline typename CompareTypes<abtypes>::supertype funcname(const atype & a, const btype & b)      \
  {                                                                                                \
...
#define NumberArray_std_binary_aa(funcname, atype)                                                 \
  template <std::size_t N, typename T>                                                             \
  inline atype funcname(const atype & a, const atype & b)                                          \
  {                                                                                                \
    atype returnval;                                                                               \
                                                                                                   \
    for (std::size_t i = 0; i != N; ++i)                                                           \
