...
MooseVariableFV<OutputType>::adGradSln(const Elem * const elem) const
{
  ...
  auto action_functor = [&grad, &volume_set, &volume, &elem_value, this](
    ...
    if (!volume_set)
    {
      ...
      if (elem_has_info)
      {
        coordTransformFactor(this->_subproblem, functor_elem.subdomain_id(), fi->elemCentroid(), coord);
        volume = fi->elemVolume() * coord;
      }
      else
      {
        coordTransformFactor(this->_subproblem, neighbor->subdomain_id(), fi->neighborCentroid(), coord);
