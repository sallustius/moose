                                         ...
                                         const Elem * lowerDElem /*=nullptr*/)
{
  ...
  // This is for the master boundary of mortar constraints
  if (_fe_problem.needNeighborBoundaryMaterial(bnd_id, _tid))
  {
  }

  void ComputeMaterialsObjectThread::onInternalSide(const Elem * elem, unsigned int side)
  {
           ...
           (neighbor->level() < elem->level())))
      {
        ...
        if (_discrete_materials[Moose::NEIGHBOR_MATERIAL_DATA].hasActiveBlockObjects(
          ...
          _neighbor_material_props.initStatefulProps(
              ...
              *elem,
              side);
        if (_materials[Moose::NEIGHBOR_MATERIAL_DATA].hasActiveBlockObjects(
                neighbor->subdomain_id(), _tid))
          _neighbor_material_props.initStatefulProps(
  ...
  void ComputeMaterialsObjectThread::onInterface(
      const Elem * elem, unsigned int side, BoundaryID bnd_id)
  {
    if (_fe_problem.needInterfaceMaterialOnSide(bnd_id, _tid))
    {
      ...
      // Do we have neighbor stateful properties or do we have stateful interface material
      // properties? If either then we need to reinit the neighbor, so at least at a minimum
      // _neighbor_elem isn't NULL!
      if (neighbor->active() && (_has_neighbor_stateful_props ||
                                 (_has_bnd_stateful_props &&
                                  _interface_materials.hasActiveBoundaryObjects(bnd_id, _tid))))
        _assembly[_tid]->reinitElemAndNeighbor(elem, side, neighbor, neighbor_side);
  ...
  void ComputeMaterialsObjectThread::join(const ComputeMaterialsObjectThread & /*y*/) {}
