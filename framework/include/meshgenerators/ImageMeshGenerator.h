...
protected:
   ...
   */
  void buildMesh2D(const std::string & filename, std::unique_ptr<MeshBase> & mesh);

  /**
   * generate() calls this helper function to build 3D ImageMeshes from stacks of images.
   */
  void buildMesh3D(const std::vector<std::string> & filenames, std::unique_ptr<MeshBase> & mesh);
